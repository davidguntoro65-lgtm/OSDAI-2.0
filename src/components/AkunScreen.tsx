...
export default function AkunScreen({ user, authToken, onLogout }: Props) {
  ...
  const menuItems = [
    { icon: User, label: 'Profil Pengguna', sub: user?.name, color: '#FF6A00', onClick: undefined },
    { icon: Key, label: 'Keamanan Akun', sub: 'Ubah kata sandi', color: '#7c3aed', onClick: () => setShowChangePassword(true) },
    { icon: Bell, label: 'Notifikasi', sub: 'Atur preferensi', color: '#0ea5e9', onClick: undefined },
    { icon: ShieldCheck, label: 'Privasi & GPS', sub: 'Izin lokasi & perangkat', color: '#10b981', onClick: undefined },
    { icon: HelpCircle, label: 'Bantuan & Dukungan', sub: 'FAQ & kontak admin', color: '#f59e0b', onClick: undefined },
    { icon: Info, label: 'Tentang OSDAI', sub: 'v2.0.0 · OSDAI Platform', color: '#64748b', onClick: undefined },
  ];
    ...
    <>
      <div className="flex-1 overflow-y-auto pb-24" style={{ background: '#1C100A' }}>
        ...
        <div className="px-4 py-3 space-y-2">
          {menuItems.map(({ icon: Icon, label, sub, color, onClick }, i) => (
            <motion.button
              key={label}
            ...
            >
              ...
              {onClick && <ChevronRight size={15} style={{ color: 'rgba(255,255,255,0.2)' }} />}
            </motion.button>
          ))}
        </div>
